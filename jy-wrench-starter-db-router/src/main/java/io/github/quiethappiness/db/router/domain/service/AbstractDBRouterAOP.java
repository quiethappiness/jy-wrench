...
import io.github.quiethappiness.db.router.config.configuration.DBRouterProperty;
...
public abstract class AbstractDBRouterAOP implements IDBRouterAOP
{
	...
	protected CalRouterResult doCalRouter(ProceedingJoinPoint jp, String dbKey)
	{
		...
		int dbCount = dbRouterProperty.getDbCount();
		int tbCount = dbRouterProperty.getTbCount();
		int size = dbCount * tbCount;
		// 改进的哈希计算：使用绝对值确保非负，并增加分布均匀性
		int hashCode = dbKeyAttr.hashCode();
		int perturbedHash = hashCode ^ (hashCode >>> 16);
		// 扰动函数
		int idx = (size - 1) & (hashCode ^ (hashCode >>> 16));
		// 库表索引
		int dbIdx = idx / tbCount + 1;
		int tbIdx = Math.abs(idx - tbCount * (dbIdx - 1)) % tbCount + 1;
		// 验证索引范围
