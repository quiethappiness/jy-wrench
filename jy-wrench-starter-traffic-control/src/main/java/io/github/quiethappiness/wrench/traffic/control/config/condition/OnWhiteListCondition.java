...
import static io.github.quiethappiness.wrench.traffic.control.config.property.WhiteListProperties.JY_WRENCH_CONFIG_TRAFFIC_WHITELIST_ENABLED;
...
public class OnWhiteListCondition extends AnyNestedCondition
{
	...
	// 条件1：检查YAML配置
	@Slf4j
	public static class OnWhiteListPropertyCondition extends SpringBootCondition
	{
		...
		public ConditionOutcome getMatchOutcome(ConditionContext context, AnnotatedTypeMetadata metadata)
		{
			...
			boolean enabled = context
				.getEnvironment()
				.getProperty(JY_WRENCH_CONFIG_TRAFFIC_WHITELIST_ENABLED, Boolean.class, true);
			if (enabled)
			{
				// log.info("通过YAML配置启用了 whitelist 功能");
				return ConditionOutcome.match("通过YAML配置启用了 whitelist 功能");
			}
			return ConditionOutcome.noMatch("未在YAML配置中启用 whitelist 功能");
		}
	}
	
	// 条件2：检查是否存在@EnableMethodExtension注解
	@Slf4j
	public static class OnWhiteListAnnotationCondition extends SpringBootCondition
	{
		@Override
		public ConditionOutcome getMatchOutcome(ConditionContext context, AnnotatedTypeMetadata metadata)
		{
			...
			{
				if (metadata.isAnnotated(EnableWhiteList.class.getName()))
				{
			...
			if (isAnnotationPresentOnAnyBean(context, EnableWhiteList.class))
			{
