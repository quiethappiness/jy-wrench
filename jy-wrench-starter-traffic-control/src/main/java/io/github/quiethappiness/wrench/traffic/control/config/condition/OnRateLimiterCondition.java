...
import static io.github.quiethappiness.wrench.traffic.control.config.property.RateLimiterProperties.JY_WRENCH_CONFIG_TRAFFIC_RATELIMITER_ENABLED;
...
public class OnRateLimiterCondition extends AnyNestedCondition
{
	...
	// 条件1：检查YAML配置
	@Slf4j
	public static class OnRateLimiterPropertyCondition extends SpringBootCondition
	{
		...
		public ConditionOutcome getMatchOutcome(ConditionContext context, AnnotatedTypeMetadata metadata)
		{
			...
			boolean enabled = context
				.getEnvironment()
				.getProperty(JY_WRENCH_CONFIG_TRAFFIC_RATELIMITER_ENABLED, Boolean.class, true);
			if (enabled)
			{
	...
	@Slf4j
	public static class OnRateLimiterAnnotationCondition extends SpringBootCondition
	{
		@Override
		public ConditionOutcome getMatchOutcome(ConditionContext context, AnnotatedTypeMetadata metadata)
		{
			...
			{
				if (metadata.isAnnotated(EnableRateLimiter.class.getName()))
				{
			...
			if (isAnnotationPresentOnAnyBean(context, EnableRateLimiter.class))
			{
